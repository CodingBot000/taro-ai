...
export default function CardSelectionScreen({
  ...
  const getCardAriaLabel = (card: CardSlot, index: number) => {
    const cardNumber = index + 1;
    if (!card.isSelected) {
      return `${cardNumber}번째 카드 선택`;
    }
    if (readingType === 'three-card' && card.selectionOrder) {
      return `${cardNumber}번째 카드 선택됨, 선택 순서 ${card.selectionOrder}`;
    }
    return `${cardNumber}번째 카드 선택됨`;
  };
          ...
          {deck.map((card, index) => {
            ...
            return (
              <button
                ...
                aria-label={getCardAriaLabel(card, index)}
